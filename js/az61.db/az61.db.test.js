					...
					for (var i = 0; i < result.rows.length; i++) {
						...
						var category = {};
	      				...
	      				if($.inArray(newCatId,categoryIds) == -1){
	      					...
	      					$('ul.testList').append('<li class="category" id="cat_'+row.catId+'"><img class="folderIcon" src="'+ imgPath +'folder_closed.png" /><span class="cat-title"">' + row.CategoryName + '</span><ul></ul></li>');
	      				}
	      				
	      				$('ul.testList').find('li#cat_'+row.catId + ' ul').append('<li class="lesson" id="lesson_'+row.LessonId+'"><input class="checkLesson" type="checkbox" value=""/><span class="lessonTitle">'+row.LessonName+'</span></li>');
					}
				}
			}
		});
